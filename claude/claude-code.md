...
* ドキュメントを作成して、それがOKになってから実行してもらう
...
* **Create CLAUDE.md**: `/init'
  ...
  * 書くとすれば、プロジェクト特有のルールや技術など、Claudeがプロジェクトを検索しても見つけられない情報だけを記述する
    * `/init'` を実行しないで手動で用意
    * 他のAIに渡り歩くときに忘れてしまうので、ちゃんと残しておく
    * 汎用的に使うものではない
...
### サンプルテンプレート
* ネット上のテンプレートを参考にしたり、AIに作ってもらってもOK
```
# Project
...
## 禁止事項
* レビューなしにコミットしないでください
* 「仮の値をセットしておいて後で調整」というアプローチは禁止

## その他お願いしておくこと
* プログラムはできるだけ機能毎に分割してモジュール化してもらう
	* 毎回全部確認して対応しなくて済むように
```
